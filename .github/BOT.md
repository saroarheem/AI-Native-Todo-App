...
| --- | --- | --- | --- |
| **Typo review & auto-fix** (`claude-typo-review.yml`) | PR opened / updated | Fix typos **in code comments and Markdown/text docs**, verify the build, commit & push to the **PR branch**, and comment | Typos in strings / UI text / identifiers / config; edits that break the build; unexpected file types; fork PRs; **merging** |
...
## 1. Typo review & auto-fix — `claude-typo-review.yml`

Scans the added/changed lines of every pull request for spelling mistakes,
sorts each into "safe to fix" vs "needs a human", and fixes the safe ones.

- **✅ Automatic — fixed and pushed to the PR branch:** typos located in **code
  comments** or in **Markdown / plain-text documentation**. These cannot change
  program behavior. Before pushing, the workflow enforces two gates:
  1. only comment/doc-bearing file types were touched
     (`*.md`, `*.txt`, `*.ts`, `*.html`, `*.css`, `*.scss`); and
  2. if any source file changed, the project **still builds** (`npm run build`) —
     a genuine comment-only edit keeps the build green.

- **🛑 Needs human approval (reported in the PR comment, never auto-applied):**
  - typos in **string literals / user-facing UI text**;
  - typos in **identifiers** (variable / function / class / property names) —
    renaming one safely means renaming it everywhere;
  - typos in configuration or any executable code token;
  - any edit that would **break the build**, or that touches an unexpected file
    type;
  - fork PRs (no write token / secrets available).

> The safe fix is pushed to the **PR branch only — never to `main`** — so PR
...
`GITHUB_TOKEN`; the two PR bots (typo and test auto-fix) additionally need
`contents: write` (declared in their workflows) to push safe fixes to PR
branches, while the triage bot needs `issues: write`.
